...
DEFAULT_SYSTEM = {
    "nodes": {
        "Rentabilidade": {"cat": "Estado", "val": 0, "expr": "Receita - Custos",
                          "desc": "Acumulado de receitas menos custos.", "x": 760, "y": 300},
        "Receita": {"cat": "Equação", "val": 0, "expr": "(Pt*Qt)*(1-Carga_Tributaria)", "desc": "Receita líquida.",
                    "x": 560, "y": 200},
        "Custos": {"cat": "Equação", "val": 0, "expr": "Custo_Fixo+Custo_Variavel_t+Emprestimos_Mensal",
                   "desc": "Custo total.", "x": 560, "y": 400},
        "Pt": {"cat": "Input", "val": 150, "expr": "", "desc": "Preço de venda.", "x": 360, "y": 130},
        "Qt": {"cat": "Equação", "val": 0, "expr": "MIN(Dpt*St, Cap)", "desc": "Demanda capturada.", "x": 360,
               "y": 270},
        "Carga_Tributaria": {"cat": "Parâmetro", "val": 0.15, "expr": "", "desc": "Percentual de impostos.", "x": 560,
                             "y": 100},
        "Custo_Fixo": {"cat": "Equação", "val": 500,
                       "expr": "500+budget_update+budget_training+budget_infra+budget_promo", "desc": "Custos fixos.",
                       "x": 360, "y": 430},
        "Custo_Variavel_t": {"cat": "Equação", "val": 0, "expr": "CVt*(1+Inflacao)", "desc": "Custo variável ajustado.",
                             "x": 360, "y": 520},
        "Emprestimos_Mensal": {"cat": "Equação", "val": 0, "expr": "Emprestimo/a_ni", "desc": "Parcela mensal.",
                               "x": 560, "y": 520},
        "Dpt": {"cat": "Equação", "val": 0, "expr": "B*Alfa*Teta_1", "desc": "Demanda Potencial.", "x": 160, "y": 270},
        "St": {"cat": "Equação", "val": 0, "expr": "EXP(-Beta_1*(Pt-Pm))/(1+Nc)", "desc": "Market share.", "x": 360,
               "y": 200},
        ...
        "CVt": {"cat": "Equação", "val": 0, "expr": "Qt*(Cp+C_Mao_Obra)", "desc": "Custo variável base.", "x": 160,
                "y": 560},
        ...
        "a_ni": {"cat": "Equação", "val": 0,
                 "expr": "((1+Tx_Juros_Emprestimo)^Prazo-1)/((1+Tx_Juros_Emprestimo)^Prazo*Tx_Juros_Emprestimo)",
                 "desc": "Fator anuidade.", "x": 760, "y": 520},
        "Tx_Juros_Emprestimo": {"cat": "Parâmetro", "val": 0.016, "expr": "", "desc": "Juros mensais.", "x": 860,
                                "y": 580},
...
#right-panel .panel-content { flex: 1; overflow-y: auto; padding: 14px 16px; }
...
.detail-eq {
  font-family: var(--mono); font-size: 12px; color: var(--teal); background: var(--bg3); padding: 8px 10px; border-radius: 6px;
  border-left: 2px solid var(--teal); word-break: break-all; line-height: 1.5;
}
  ...
  <div id="right-panel">
    ...
    <div class="panel-content" id="detail-content">
      <div class="placeholder-text">
        <div class="icon">⬡</div>
        <p>Clique em um nó para ver seus detalhes, equações e relações causais.</p>
      </div>
    </div>
    <div class="legend">
...
<div class="modal-overlay" id="modal-add-node">
  <div class="modal">
    ...
    <div class="form-row"><label class="form-label">Nome da Variável *</label><input class="form-input" id="new-node-name" placeholder="ex: taxa_crescimento"></div>
    ...
    <div class="form-row"><label class="form-label">Valor Inicial</label><input class="form-input" id="new-node-val" type="number" placeholder="0" value="0"></div>
    <div class="form-row"><label class="form-label">Equação (opcional)</label><textarea class="form-textarea" id="new-node-eq" placeholder="ex: price * demand_captured * (1 - tax_rate)"></textarea></div>
    <div class="form-row"><label class="form-label">Descrição *</label><textarea class="form-textarea" id="new-node-desc" placeholder="Descreva o que esta variável representa..."></textarea></div>
...
<div class="modal-overlay" id="modal-add-link">
  <div class="modal">
    ...
    <div class="link-hint" id="link-hint-text">Selecione a variável de destino no diagrama ou escolha abaixo.</div>
...
<script>
// ============================================================
// DATA MODEL & STREAMLIT COMMUNICATION
// ============================================================
let SYSTEM = { nodes: {}, links: [] };
...
let scale = 0.75;
const catColorClass = { Estado:"cat-estado", Equação:"cat-equacao", Parâmetro:"cat-parametro", Ambiente:"cat-ambiente", Input:"cat-input" };

function sendToStreamlit() {
  if (window.parent && window.parent.Streamlit) {
    window.parent.Streamlit.setComponentValue(JSON.stringify(SYSTEM));
  }
}
...
function renderAll() {
  ...
  for (const [id, node] of Object.entries(SYSTEM.nodes)) {
    ...
    const cc = catColorClass[node.cat] || 'cat-estado';
    el.innerHTML = `<div class="node-box ${cc}">${id}</div>`;
...
function renderEdges() {
  ...
  for (const link of SYSTEM.links) {
    const fromNode = SYSTEM.nodes[link.from];
    const toNode = SYSTEM.nodes[link.to];
    if (!fromNode || !toNode) continue;
    const fromEl = document.getElementById('node-' + link.from);
    const toEl = document.getElementById('node-' + link.to);
    if (!fromEl || !toEl) continue;

    const fx = (fromNode.x * scale + panX);
    const fy = (fromNode.y * scale + panY);
    const tx = (toNode.x * scale + panX);
    const ty = (toNode.y * scale + panY);

    const dx = tx - fx, dy = ty - fy;
    const dist = Math.sqrt(dx*dx + dy*dy) || 1;
    const ux = dx/dist, uy = dy/dist;

    const halfFw = (fromEl.offsetWidth * scale) / 2 + 2;
    const halfFh = (fromEl.offsetHeight * scale) / 2 + 2;
    const halfTw = (toEl.offsetWidth * scale) / 2 + 2;
    const halfTh = (toEl.offsetHeight * scale) / 2 + 2;

    const x1 = fx + ux * halfFw;
    const y1 = fy + uy * halfFh;
    const x2 = tx - ux * halfTw;
    const y2 = ty - uy * halfTh;
    ...
    const marker = link.sign === '+' ? 'url(#arr-pos)' : 'url(#arr-neg)';
...
function onNodeClick(e, id) {
  ...
  if (currentTool === 'link') {
    if (!linkSource) {
      ...
      document.getElementById('link-hint-text').textContent = `Origem: "${id}" — Agora clique no nó de destino.`;
      document.getElementById('link-from').value = id;
      const sel = document.getElementById('link-to');
      sel.innerHTML = Object.keys(SYSTEM.nodes).filter(n => n !== id).map(n => `<option value="${n}">${n}</option>`).join('');
...
function renderDetailPanel(id) {
  ...
  const outgoing = SYSTEM.links.filter(l => l.from === id);
  const incoming = SYSTEM.links.filter(l => l.to === id);
  const catColor = { Estado:'#6a8aff', Equação:'#ff9090', Parâmetro:'#c090ff', Ambiente:'#60ffa0', Input:'#ffb060' };
  const col = catColor[node.cat] || '#aaa';
  let html = `<div class="node-detail-name">${id}</div>`;
  html += `<div class="node-detail-cat" style="color:${col}">${node.cat}</div>`;
  html += `<div class="detail-label">Descrição</div><div class="detail-desc">${node.desc}</div>`;
  html += `<div class="detail-label">Valor Atual</div><div class="detail-desc" style="font-family:var(--mono);color:var(--gold)">${node.val}</div>`;
  if (node.expr) html += `<div class="detail-label">Equação</div><div class="detail-eq">${node.expr}</div>`;
  if (incoming.length) {
    html += `<div class="detail-label">Causas (entrada)</div>`;
    incoming.forEach(l => {
      html += `<div class="relation-item"><span class="rel-arrow" style="color:${l.sign==='+'?'#52c97a':'#e06060'}">⟶</span><span class="rel-sign ${l.sign==='+' ? 'rel-pos':'rel-neg'}">${l.sign}</span><span style="color:var(--text2)">${l.from}</span></div>`;
    });
  }
  if (outgoing.length) {
    html += `<div class="detail-label">Efeitos (saída)</div>`;
    outgoing.forEach(l => {
      html += `<div class="relation-item"><span class="rel-sign ${l.sign==='+' ? 'rel-pos':'rel-neg'}">${l.sign}</span><span style="color:var(--text2)">${l.to}</span></div>`;
    });
  }
  html += `<div style="margin-top:16px;display:flex;gap:8px">
    <button class="btn-icon" style="font-size:12px" onclick="openEditNodeModal('${id}')">✎ Editar Variável</button>
    <button class="btn-icon" style="font-size:12px" onclick="openLinkFromSelected()">+ Ligação</button>
    <button class="btn-icon" style="font-size:12px;color:var(--coral)" onclick="deleteNode('${id}')">✕ Excluir</button>
  </div>`;
...
document.addEventListener('mousemove', (e) => {
  if (dragNode) {
    const wrap = document.getElementById('canvas-wrap');
    const rect = wrap.getBoundingClientRect();
    const nx = (e.clientX - rect.left - panX) / scale - dragOffX;
    const ny = (e.clientY - rect.top - panY) / scale - dragOffY;
    SYSTEM.nodes[dragNode].x = nx;
    SYSTEM.nodes[dragNode].y = ny;
    isDragging = true;
    renderAll();
  } else if (isPanning) {
...
function openLinkFromSelected() {
  if (selectedNode) {
    ...
    const sel = document.getElementById('link-to');
    sel.innerHTML = Object.keys(SYSTEM.nodes).filter(n => n !== selectedNode).map(n => `<option value="${n}">${n}</option>`).join('');
    document.getElementById('modal-add-link').classList.add('open');
  }
}
...
function resetLayout() {
  const keys = Object.keys(SYSTEM.nodes);
  const cols = Math.ceil(Math.sqrt(keys.length * 1.5));
  keys.forEach((k, i) => {
    SYSTEM.nodes[k].x = 120 + (i % cols) * 160;
    SYSTEM.nodes[k].y = 120 + Math.floor(i / cols) * 120;
  });
  panX = 40; panY = 40; scale = 0.7;
  renderAll();
  sendToStreamlit();
}
...
function confirmLink() {
  ...
  const exists = SYSTEM.links.find(l => l.from === from && l.to === to);
  if (exists) { exists.sign = sign; exists.desc = desc; }
  else SYSTEM.links.push({ from, to, sign, desc });
...
// Carrega o modelo a partir do elemento <script> com os dados iniciais
(function() {
...
with tab_cld:
    ...
    html_content = get_simulator_html().replace('__INITIAL_MODEL__', model_json)

    updated_model = components.html(html_content, height=700, scrolling=False)

    if updated_model:
        try:
            new_system = json.loads(updated_model)
            if new_system != SYSTEM:
                st.session_state.system = new_system
                save_system(new_system)
                for k, v in new_system["nodes"].items():
...
with tab_sim:
    ...
    with col_kpi1:
        st.markdown(
            f'<div class="card"><div class="kpi-label">Rentabilidade</div><div class="kpi-value">R$ {rent / 1000:.1f}k</div></div>',
            unsafe_allow_html=True)
    with col_kpi2:
        st.markdown(
            f'<div class="card"><div class="kpi-label">Receita Ciclo</div><div class="kpi-value">R$ {receita:.0f}</div></div>',
            unsafe_allow_html=True)
    with col_kpi3:
        st.markdown(
            f'<div class="card"><div class="kpi-label">Qt Atendida</div><div class="kpi-value">{qt:.0f}</div></div>',
            unsafe_allow_html=True)
    with col_kpi4:
        st.markdown(
            f'<div class="card"><div class="kpi-label">Market Share</div><div class="kpi-value">{st_mk * 100:.1f}%</div></div>',
            unsafe_allow_html=True)
            ...
            for nid, node in SYSTEM["nodes"].items():
                if node.get("expr"):
                    try:
                        ...
                        result = 0.0
                    if node["cat"] == "Estado":
                        new_vals[nid] = snapshot[nid] + result
                    else:
                        new_vals[nid] = result
                else:
    ...
    if st.session_state.sim_history:
        ...
        fig1 = go.Figure()
        fig1.add_trace(
            go.Bar(x=cycles, y=rent_vals, marker_color=['#4a6aee' if v >= 0 else '#e06060' for v in rent_vals]))
        ...
        fig2 = go.Figure()
        fig2.add_trace(
            go.Bar(x=cycles, y=profit_vals, marker_color=['#52c97a' if v >= 0 else '#e06060' for v in profit_vals]))
